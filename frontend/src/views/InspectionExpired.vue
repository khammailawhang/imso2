                ...
                <v-col cols="12" xl="12" lg="12" md="12" sm="12">
                    <v-hover v-slot:default="{ hover }" open-delay="200">
                        <v-card :elevation="hover ? 0 : 0" class="mx-auto">
                            <v-card-title>
                                <h5>{{$t("Report.InspectionExpired")}}</h5>
                                <v-spacer />
                                <v-btn-toggle dense rounded>
                                    ...
                                    <v-btn depressed color="#3d5AFE" dark v-on="on" width="80px" class="white--text">
                                        <download-csv :data="inspections" name="Inspection_Expired.csv">{{$t("Report.Download")}}</download-csv>
                                    </v-btn>
                                </v-btn-toggle>
                            </v-card-title>
                            <v-card-subtitle>
                                <v-row>
                                    ...
                                    <v-col cols="12" xl="2" lg="2" md="4" sm="4">
                                        <v-select :items="inspections" item-text="PName" :label="$t('Province.Name')" dense color="#3d5AFE" v-model="provinceFilterValue"></v-select>
                                    </v-col>
                                    <v-col cols="12" xl="2" lg="2" md="4" sm="4">
                                        <v-select :items="inspections" item-text="TRName" :label="$t('Inspection.inspectonseach')" dense color="#3d5AFE" v-model="typeplactnoFilterValue"></v-select>
                                    </v-col>
                                    <v-col cols="12" xl="2" lg="2" md="4" sm="4">
...
export default {
    ...
    methods: {
        ...
        initialize() {
            this.axios
                .get("/api/inspection/expired_date/branch_id/" + this.$route.query.branch_id)
                .then(response => {
        ...
        fetchRecords() {
            if (this.start_date != "" && this.end_date != "") {
                this.axios
                    .get("/api/inspection/expired/branch_id/" + this.$route.query.branch_id, {
                        params: {
