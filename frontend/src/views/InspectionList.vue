                    ...
                    <v-card flat color="white">
                        ...
                        <v-card-subtitle>
                            <v-row>
                                ...
                                <v-col cols="12" xl="4" lg="4" md="4" sm="4">
                                    <v-select :items="inspections" class="mr-4" item-text="TRName" :label="$t('Inspection.inspectonseach')" dense v-model="typeplactnoFilterValue"></v-select>
                                </v-col>
                                <v-col cols="12" xl="4" lg="4" md="4" sm="4">
                                    <v-text-field dense class="mr-4" v-model="platcnoFilterValue" :label="$t('Register.Platcno')" type="text"></v-text-field>
                                </v-col>
                            </v-row>
                        </v-card-subtitle>
                        <v-card-text>
                            <v-data-table :headers="headers" :items="inspections" :search="search">
                                <template v-slot:item.brk_m="{ item }">
                                    <v-tooltip left color="#00E676">
                                        <template v-slot:activator="{ on }">
                                            <v-btn depressed small :color="status(item.brk_m)" v-on="on" v-if="
                     item.brk_m === 'true' &&
                     item.brk_s === 'true' &&
                     item.cy_c === 'true' &&
                     item.re_m === 'true' &&
                     ...
                     item.wiper === 'true'">
                                            ...
                                            <v-btn small depressed :color="status(
                     item.brk_m === 'false' &&
                     item.brk_s === 'false' &&
                     item.cy_c === 'false' &&
                     item.re_m === 'false' &&
                     ...
                     item.wiper === 'false')" v-else>
                                ...
                                <template v-slot:item.pint="{ item }">
                                    <v-tooltip left color="indigo">
                                        <template v-slot:activator="{ on }">
                                            <v-btn small :color="getColorpint(item.pint)" @click="createPDF(item)">
                                                <v-icon color="#fff" v-on="on" small>mdi-printer</v-icon>
                                    ...
                                    <v-tooltip left color="indigo">
                                        <template v-slot:activator="{ on }">
                                            <v-btn small :color="getColorpint(item.pint2)">
                                                <v-icon color="#fff" v-on="on" small @click="createPDF2(item)">mdi-printer</v-icon>
                                            </v-btn>
                                        </template>
                                    ...
                                    <v-tooltip left color="indigo">
                                        <template v-slot:activator="{ on }">
                                            <v-btn small color="indigo" dark v-on="on">
                                                <v-icon small @click="editItem(item.inspection_id)">mdi-pencil</v-icon>
                                            </v-btn>
                                        </template>
                                        <span>{{ $t("Inspection.new") }}</span>
                                    </v-tooltip>
                                </template>
...
export default {
    ...
    async created() {
        let res = await axios.get("/api/inspection/branch_id/" + this.$route.query.branch_id);
    ...
    methods: {
        ...
        async deleteItem(inspection_id, index) {
            ...
            if (confirm('Ayre you sure delete ?')) {
                let res = this.axios.delete('/api/inspection/delete/' + inspection_id).then()
                if (res) {
